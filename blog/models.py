...
class Category(models.Model): #个人文章分类表
    nid = models.AutoField(primary_key=True)
...
class Tag(models.Model): #个人文章标签表
    nid = models.AutoField(primary_key=True)
    title = models.CharField(verbose_name='标签标题',max_length=64)
    blog = models.ForeignKey(verbose_name='所属博客',to='Blog',to_field='nid',on_delete=models.CASCADE)
    def __str__(self):
        return self.title
...
class Article(models.Model): #文章表
    ...
    desc = models.CharField(verbose_name='文章描述',max_length=260)
    create_time = models.DateTimeField(verbose_name='创建时间',auto_now_add=True)
...
class Article2Tag(models.Model): #文章和标签关联表
    nid = models.AutoField(primary_key=True)
...
class Comment(models.Model): #评论表
    nid = models.AutoField(primary_key=True)
    user = models.ForeignKey(verbose_name='评论文章',to='UserInfo',to_field='nid',on_delete=models.CASCADE)
    article = models.ForeignKey(verbose_name='评论者',to='Article',to_field='nid',on_delete=models.CASCADE)
